...
| Batch'te all-or-nothing ön doğrulama (LLM öncesi fail-closed) | Bir dosya bozuksa hiçbir dosya LLM'e gitmez. Extraction/evaluation tüm CV'leri üretemezse, kısmi sıralama yerine kontrollü bir hata döner. |
| `TopCandidate`/`MultiAnalysisResponse` şemalarında `extra="forbid"` | Çıktı JSON sözleşmesini kazayla bozacak ekstra bir alan validation hatası fırlatır. Şema sapması, çıktı üretilir üretilmez (Pydantic model construction anında) runtime'da yakalanır — Python'da derleme zamanı denetimi yoktur, bu bir "compile time" garantisi değildir. |
| Nominal batch başına iki LLM çağrısı (CV başına değil) | Önce 5 CV tek çağrıda 5 profile çevrilir; sonra bu profiller tek çağrıda değerlendirilir. Ham metin skorlama prompt'una girmez. Model bir evaluation belgesini atlar/tekrarlarsa yalnız o profil tekli şemayla tamamlanır; geçerli batch sonuçları yeniden üretilmez. |
| Telegram/LLM I/O `asyncio` ile, senkron PDF parse `asyncio.to_thread` ile | Telegram (`python-telegram-bot`) ve LLM (`httpx`) çağrıları I/O-bound'dur, doğrudan `asyncio` ile yürütülür. PyMuPDF senkron bir kütüphanedir; event loop'u bloklamaması için `asyncio.to_thread` ile (kendi arka planında bir thread pool kullanarak) ayrı bir thread'e atılır — "thread pool yerine asyncio" değil, ikisi birlikte, doğru katmanda kullanılır. |
