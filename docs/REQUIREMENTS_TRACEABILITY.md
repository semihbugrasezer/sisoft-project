...
| FR-11 | Puanlama/analiz/filtreleme **yalnızca** ortak JSON üzerinden | `cv_analysis_service.py` — evaluator prompt'u yalnız `profile.model_dump_json()` alır. Ham metin yalnız çıktıdan sonra kaynak-dışı kanıtı sıfırlayan trust-boundary kontrolünde kullanılır; skora bilgi eklemez. | Aynı test: ham kaynak metnin ikinci prompt'ta bulunmadığını doğrular; grounding testleri kaynak-dışı skorun sıfırlandığını doğrular |
