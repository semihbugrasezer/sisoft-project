...
| FR-04 | Kullanıcının tanımladığı kriterlerin tamamı korunur; skorlama bunlara göre yapılır | `criteria_service.py` → intent sonrasında özel extraction + grounding + serbest-metin liste tamlığı | `tests/test_criteria_service.py::test_natural_language_criteria_uses_dedicated_extraction_before_save`, `::test_free_text_list_separators_do_not_allow_partial_criteria` |
...
| FR-10 | Ham PDF metni doğrudan analiz edilmez; önce ortak JSON şeması | `models.py::NormalizedCandidateDraft` genel `CandidateProfile` ile dinamik `criterionEvidence[]` ledger'ını taşır; Python exact span ve kararlı evidence ID üretir; PDF adapter çok kolonlu cümleleri block sırasıyla korur | `tests/test_models.py::test_normalized_candidate_draft_requires_criterion_evidence_from_extractor`, `tests/test_grounding.py`, `tests/test_pdf_parser.py::test_multicolumn_text_keeps_block_sentences_contiguous` |
| FR-11 | Puanlama/analiz/filtreleme **yalnızca** ortak JSON üzerinden | `cv_analysis_service.py` — raw source doğrulaması normalization aşamasında biter; tek batch semantic verifier ledger'ı sınıflandırır; evaluator ve `_normalize_scores` yalnız doğrulanmış evidence ID'lerini kullanır | `tests/test_cv_analysis_service.py::test_batch_uses_one_extraction_verifier_and_evaluation_call_without_raw_leak`; irrelevant, cross-criterion, hallucination, contradiction ve injection regresyonları |
...
| FR-13 | Dosyalar asenkron/paralel işlenir | `batch_analysis_service.py::_validate_all_or_abort` (`asyncio.gather`), `cv_analysis_service.py::extract_text` (`asyncio.to_thread`) | `tests/test_batch_analysis.py`; bkz. [Mimari](./ARCHITECTURE.md#eşzamanlılık-modeli) |
...
| NFR-04 | Kilitlenmeyen asenkron mesajlaşma | `router.py` → `concurrent_updates(8)`, `handlers.py` → chat_id bazlı iki ayrı `asyncio.Lock` ailesi | `tests/test_router.py` (eşzamanlı update kabulü), `tests/test_handlers.py::test_same_chat_messages_are_processed_in_arrival_order` ve `::test_different_chats_are_not_serialized_against_each_other`; [Mimari](./ARCHITECTURE.md#eşzamanlılık-modeli); canlı koşu: batch sırasında bot yanıt vermeye devam etti |
...
| EVAL-02 | **PDF Doğrulama & LLM Extraction Kalitesi** | `validate_and_extract_text` geçersiz belgeleri LLM'den önce keser. Geçerli ham metin ve dinamik kriterler `NormalizedCandidateDraft` şemasına çıkarılır; Python exact span/ID üretir, ayrı verifier semantik bağı sınıflandırır ve evaluator yalnız doğrulanmış ID seçer. | `tests/test_pdf_parser.py`, `tests/test_cv_layouts.py`, `tests/test_models.py`, `tests/test_grounding.py`, `tests/test_cv_analysis_service.py`; beş farklı mock şablon ve dört geçersiz fixture; [Canlı Doğrulama](./VALIDATION.md) |
| EVAL-03 | **Asenkron Süreç & Bağlam Yönetimi** | `concurrent_updates(8)` farklı update'leri eşzamanlı kabul eder; PyMuPDF ve SQLite çağrıları `asyncio.to_thread` ile event loop dışına taşınır. Aynı sohbette mesaj/kriter/reset sırası `chat_id` bazlı metin kilidiyle korunurken CV analizi ayrı kilit ailesinde çalışır. Son 40 mesaj sıcak pencere, eski bölüm rolling summary olarak backend'de tutulur. | `tests/test_router.py`, `tests/test_handlers.py`, `tests/test_chat_service.py`, `tests/test_sqlite_repo.py`; özellikle batch sürerken sohbet, aynı sohbet sırası ve reset yarış testleri; gerçek Telegram rolling-summary/batch koşuları; [Mimari](./ARCHITECTURE.md#eşzamanlılık-modeli) |
| EVAL-04 | **Vibe Coding Hakimiyeti** | AI araçlarının rolü, insan onay kapıları ve reddedilen öneriler kaydedilmiştir. Katman bağımlılıkları, LLM portu, Pydantic doğrulama, async kilitler, hata hiyerarşisi ve fallback davranışları ayrı belgelerde gerekçelendirilir. Canlı koşuda bulunan dil sızıntısı, Markdown fallback, timeout, grounding ve yarış koşulları regresyon testleriyle kapatılmıştır. | [AI Destekli Geliştirme](./AI_ASSISTED_DEVELOPMENT.md), [Mimari](./ARCHITECTURE.md), [Canlı Doğrulama](./VALIDATION.md) |
...
| Vector database / RAG | Uygulanmadı | Retrieval gerektiren bir belge koleksiyonu yok; her CV oturum içinde yükleniyor ve normalize profile dönüşüyor. Bkz. [Mimari](./ARCHITECTURE.md#temel-tasarım-kararları). |
